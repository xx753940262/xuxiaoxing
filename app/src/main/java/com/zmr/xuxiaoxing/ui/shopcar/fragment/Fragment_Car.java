package com.zmr.xuxiaoxing.ui.shopcar.fragment;
...
import com.zmr.xuxiaoxing.ui.shopcar.activity.QueRenDingDanActivity;
import com.zmr.xuxiaoxing.ui.shopcar.adapter.Adapter_Shopcart;
import com.zmr.xuxiaoxing.ui.shopcar.model.CarDel;
import com.zmr.xuxiaoxing.ui.shopcar.model.CarList;
import com.zmr.xuxiaoxing.ui.shopcar.model.GoodsInfo2;
import com.zmr.xuxiaoxing.ui.shopcar.model.M_QueRenDingDanGoods;
import com.zmr.xuxiaoxing.ui.shopcar.model.StoreInfo;
 ...
 */
public class Fragment_Car extends BaseFragment implements Adapter_Shopcart.CheckInterface,
        Adapter_Shopcart.ModifyCountInterface, Adapter_Shopcart.GroupEdtorListener, OnMessageResponse ,View.OnClickListener{

    public static Fragment_Car instance;
    ImageView back;
    TextView title;
    TextView subtitle;
    LinearLayout topBar;
...
//        initDatas();
    ...
    public static Fragment_Car getSingleton() {
        if (instance == null) {
            synchronized (BaseApplication.class) {
                if (instance == null) {
                    instance = new Fragment_Car();
                }
            }
        }
        return instance;
    ...
    public void initView(View view, LayoutInflater inflater) {
        ...
        mSwipyRefreshLayout.setOnRefreshListener(new SwipyRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh(int index) {
                ...
                BaseApi.carList(Fragment_Car.this, sHelper.getString(BaseConstant.TOKEN), sHelper.getString(BaseConstant.UID));
            }
...
//            ProgressDialogUtils.showProgressDialog(getActivity(), "数据加载中...");
    ...
    private void initEvents() {
        mAdapterShopcart = new Adapter_Shopcart(groups, children, getActivity(), sHelper, this);
        ...
        for (int i = 0; i < groups.size(); i++) {
            ...
            for (int j = 0; j < childs.size(); j++) {
                if (childs.get(j).isChoosed()) {
                    BaseApi.carDel(Fragment_Car.this, sHelper.getString(BaseConstant.TOKEN), childs.get(j).getRecId() + "", sHelper.getString(BaseConstant.UID));
    ...
    protected void collect() {
        ...
        for (int i = 0; i < groups.size(); i++) {
            ...
            for (int j = 0; j < childs.size(); j++) {
                if (childs.get(j).isChoosed()) {
                    BaseApi.productCollectCar(Fragment_Car.this, childs.get(j).getGoodsId(), sHelper.getString(BaseConstant.UID), sHelper.getString(BaseConstant.TOKEN));
                }
            }
...
//                                checkAll();
            ...
            ToastUtil.showMessage(getActivity(), "分享成功");
...
//        }
    ...
    public void onMessageResponse(String url, JSONObject jo) throws JSONException {
        if (url.equals(BaseConstant.CART_LIST) && jo != null) {
            ...
            groups.clear();
            children.clear();
            CarList m_cart_one = new CarList(jo.toString());
            if (m_cart_one.getCode().equals("200")) {

                List<CarList.DataBean.GoodsDataBean> contentBeanList = m_cart_one.getData().getGoods_data();

                if (contentBeanList != null) {
                    for (int i = 0; i < contentBeanList.size(); i++) {
                        groups.add(new StoreInfo(i + "", contentBeanList.get(i).getShop_name(), contentBeanList.get(i).getShop_id()));
                        List<GoodsInfo2> products = new ArrayList<GoodsInfo2>();


                        List<CarList.DataBean.GoodsDataBean.ProductsBean> goodsList = contentBeanList.get(i).getProducts();
                        if (goodsList.size() > 0) {
                            ...
                            for (int j = 0; j < goodsList.size(); j++) {
                                products.add(new GoodsInfo2(goodsList.get(j).getCart_id(), goodsList.get(j).getProduct_id(), goodsList.get(j).getProduct_name(),
                                        ...
                                        Double.parseDouble(goodsList.get(j).getPrice()), Integer.parseInt(goodsList.get(j).getGoods_num()), "", "",
                                        goodsList.get(j).getProduct_image(), Double.parseDouble(goodsList.get(j).getSale_price()), "", "", goodsList.get(j).getPackageX(), goodsList.get(j).getShipping_fee()));
                ...
                ToastUtil.showMessage(getActivity(), m_cart_one.getMsg());
            }
            setCartNum();
        ...
        if (url.equals(BaseConstant.CART_UPDATENUM) && jo != null) {
            CarDel carDel = new CarDel(jo.toString());
            if (carDel.getCode().equals("200")) {
                ToastUtil.showMessage(getActivity(), "数量修改成功");
            }
        }
